...
<body class="hold-transition sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed" style="background-color: var(--bs-secondary-bg);">
  <div class="wrapper">
    ...
    <div class="preloader flex-column justify-content-center align-items-center" style="background-color: var(--bs-secondary-bg);">
      <img class="animation__wobble" src="<?= base_url('dist/img/logopoltek.png') ?>" alt="Logo Poltek" height="60" width="60">
    </div>
  ...
  <script>
    $(document).ready(function() {
      ...
      $('div#konfirmasi-hapus').on('show.bs.modal', function(event) {
        let button = $(event.relatedTarget) // Button that triggered the modal
        let id = button.data('id') // Extract info from data-* attributes
        let modal = $(this)
        modal.find('.modal-title').text('Konfirmasi Hapus data ' + id)
        modal.find('a#btn-hapus').attr('href', '<?= base_url('/data/delete/') ?>' + id)
      })

      $('div#modal-edit').on('show.bs.modal', function(event) {
        let button = $(event.relatedTarget) // Button that triggered the modal
        let id = button.data('id') // Extract info from data-* attributes
        ...
        modal.find('input#jenis').val(jenis)
        modal.find('input#point').val(point)
        modal.find('textarea#keterangan').val(keterangan)
        // modal.find('.modal-title').text('Konfirmasi Edit data ' + id)
        // modal.find('a#btn-edit').attr('href', '<?= base_url('/data/edit/  ') ?>' + id)
      })
    })
  </script>
</body>
